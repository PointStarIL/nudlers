...
export function prepareCredentials(vendor, rawCredentials) {
  ...
  if (vendor === 'hapoalim') {
    ...
    // Yahav login form requires a separate nationalID (#pinno) field.
    // Without it puppeteer's keyboard.type(undefined) throws "text is not iterable".
    // Most users have their national ID stored as the username — fall back to that
    // so existing setups work without UI changes.
    if (vendor === 'yahav') {
      credentials.nationalID = String(username || id || rawCredentials.id_number || '');
    }
